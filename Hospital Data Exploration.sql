...
--Q3 Categorize patients into age groups and calculate the average bill amount for eah age band (Assume age at the time of visit based on VisitDate)

WITH CTE_PatientAge AS (
SELECT v.VisitID, v.BillAmount,
CASE WHEN DATEDIFF(YEAR, p.DOB, v.VisitDate) < 18 THEN '0-17'
...
COUNT(v.VisitID) AS TotalNumOfVisits
FROM PatientVisits v
...
JOIN DIM_Department_Clean d
ON v.DepartmentID = d.DepartmentID
GROUP BY d.DepartmentCategory, d.DepartmentName ) t
...
AVG(v.WaitTimeMinutes) AS AvgWaitTime
FROM PatientVisits v
JOIN DIM_Department_Clean d
ON v.DepartmentID = d.DepartmentID
GROUP BY d.DepartmentName
...
FROM (
SELECT
CASE WHEN DATENAME(Weekday, VisitDate) IN ('Saturday', 'Sunday') THEN 'Weekend'
ELSE 'Weekday' END AS DayType
FROM PatientVisits ) t
GROUP BY DayType

--Q8 For each month, calculate total visits and a running cumulative total of visits

WITH CTE_MonthlyVisits AS (
SELECT DATEFROMPARTS(YEAR(VisitDate), MONTH(VisitDate), 1) AS MonthStart,
COUNT(*) AS TotalVisits
FROM PatientVisits 
...
--Q9 Find the doctors with the hghest average satisfaction score (minimum 100 visits)
...
RANK() OVER (PARTITION BY d.DiagnosisName ORDER BY COUNT(*) DESC) AS RankNum
FROM PatientVisits v
